...
from layers3D import SpatialTransformer, ResizeTransform, VecInt, conv_block, predict_flow, conv2D, MatchCost,\
    DoubleConv, Down, Up, Up2, Up3, OutConv, conv3D
...
class Seg_Decoder_2D(nn.Module):
    def __init__(self, inshape, n_classes):
        super(Seg_Decoder_2D, self).__init__()

        # ensure correct dimensionality
        ndims = len(inshape)
        assert ndims in [1, 2, 3], 'ndims should be one of 1, 2, or 3. found: %d' % ndims

        self.decoder = Seg_Decoder_2D()
...
class Upper(nn.Module):
    def __init__(self, inshape, n_channels=1):
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor):
        ...
        return y_source, pos_flow, encoded_src, encoded_tgt
...
class Architecture3(nn.Module):
    def __init__(self, inshape, n_channels=1, n_classes=14):
        ...
        self.upper = Upper(inshape, n_channels)
        self.lower = Seg_Decoder(inshape, n_classes)
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor):
        ...
        # concatenate inputs and propagate unet
        y_source, pos_flow, encoded_src, encoded_tgt = self.upper(source, target)
        
        # return y_source, pos_flow
        s_logits = self.lower(encoded_src)
        t_logits = self.lower(encoded_tgt)
...
class Seg(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        encoded_x = self.fea_extractor(x)

        logits = self.Seg_Decoder(encoded_x)
...
class Architecture2(nn.Module):
    def __init__(self, inshape, n_channels=1, n_classes=14):
        ...
        self.fea_extractor = Feature_Extractor(n_channels)
        self.Decoders = Decoders(inshape, n_classes)
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor):
        ...
        y_source, pos_flow, s_logits, t_logits = self.Decoders(source, encoded_src, encoded_tgt)
